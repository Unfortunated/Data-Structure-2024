...
public class BST {
	...
	public void makeBalancedTree() {
		...
		if (!this.isBalanced(this.root)) {
			...
			inOrder(storage,this.root);
			//build tree by helper function
			this.root = makeBalancedTree(storage, 0, storage.size()-1);
		}
	}
	
	//helper function to store sorted data
	public void inOrder(ArrayList<Integer> storage, BSTNode n) {
		if (n == null) {
			return;
		}
		inOrder(storage, n.left);
		storage.add(n.data);
		inOrder(storage, n.right);
	}
	
	//helper function to build tree
	public BSTNode makeBalancedTree(ArrayList<Integer> storage, int start, int end) {
		//condition to exit recursion
		if (start > end) {
			return null;
		}
		//find middle element to be parent
		int mid = (start + end)/2;
		//initialize parent
		BSTNode root = new BSTNode(storage.get(mid));
		//recursion to connect all nodes
		root.left = makeBalancedTree(storage, start,mid - 1);
		root.right = makeBalancedTree(storage, mid + 1, end);
		return root;
	}
