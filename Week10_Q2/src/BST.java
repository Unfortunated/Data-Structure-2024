...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		//TODO
		//FILL CODE
		//if subtree is an empty tree do nothing
		if (subtree.root == null) {
			return;
		}
		//if root is null subtree become this tree
		if (this.root == null) {
			this.root = subtree.root;
			this.size += subtree.size;
		}
		//maximum of subtree more than minimum of tree then not valid BST then do nothing
		...
		((TreeIterator) findMin(this.root)).currentNode.left = subtree.root;
		subtree.root.parent =((TreeIterator) findMin(this.root)).currentNode;
		this.size += subtree.size;
	}
